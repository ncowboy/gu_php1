...
{
  $items = [];
  $sql = "SELECT products_in_cart.*, products.name, products.price, products.img
          FROM products_in_cart 
          INNER JOIN products
          ON products_in_cart.product_id = products.id
          WHERE cart_id={$id}";
  $result = result(connect(), $sql);
  while ($row = mysqli_fetch_assoc($result)) {
    $items[] = $row;
